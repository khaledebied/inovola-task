...
class LocalizedNameHelper {
  ...
  LocalizedNameHelper copyWith({String? currentLanguage, String? defaultLanguage}) {
    return LocalizedNameHelper(currentLanguage: currentLanguage ?? this.currentLanguage, defaultLanguage: defaultLanguage ?? this.defaultLanguage);
  }
  ...
  String getCurrentLocalizedName(Map<String, dynamic> localizedString) {
    if (localizedString.isEmpty) {
      ...
      if (localizedString[currentLanguage] != null && localizedString[currentLanguage].toString().trim().isNotEmpty) {
        return localizedString[currentLanguage]!;
      } else if (localizedString[defaultLanguage] != null && localizedString[defaultLanguage].toString().trim().isNotEmpty) {
        return localizedString[defaultLanguage]!;
      } else {
        return localizedString.values.firstWhereOrNull((element) => (element != null && element.toString().trim().isNotEmpty)) ?? "";
      }
