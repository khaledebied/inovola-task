...
import 'package:expense_tracker/core/helpers/storage_helper.dart';
...
class GlobalNotification {
  ...
  Future<void> setupNotification() async {
    ...
    if (settings.authorizationStatus == AuthorizationStatus.authorized) {
      // if( await getIt<Utilities>().isSimulator()){
      //   await Future.delayed(const Duration(seconds: 1));
  ...
  Future<void> _showLocalNotification(RemoteMessage? message) async {
    ...
    _flutterLocalNotificationsPlugin.show(DateTime.now().microsecond, "${message.notification?.title}", "${message.notification?.body}", platform,
