...
import 'package:expense_tracker/core/widgets/base_form_option/sheet/option_sheet_content.dart';
...
abstract class AppBottomSheets {
  ...
  static Future<T?> showScrollableBodyFixedHeaderSheet<T>({
    ...
    return showModalBottomSheet<T>(context: context, backgroundColor: Colors.transparent, isDismissible: true, isScrollControlled: true, enableDrag: true, builder: builder);
  }

  static Future<T?> showScrollableBody<T>({required BuildContext context, required WidgetBuilder builder, BoxConstraints? constraints}) {
  ...
  static Future<void> showOptionsBottomSheet<T>({
    ...
    await showModalBottomSheet(
      ...
      shape: OutlineInputBorder(borderRadius: Dimens.sheetBorderRadius, borderSide: BorderSide.none),
      builder: (context) {
...
class SheetActionButton {
  ...
  factory SheetActionButton.cancel() {
    return SheetActionButton(title: Translate.s.cancel, onPressed: () => GetIt.instance<AppRouter>().pop());
  }
}
