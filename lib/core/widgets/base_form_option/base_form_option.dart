...
import 'package:expense_tracker/core/bloc/value_state_manager/value_state_manager_import.dart';
import 'package:expense_tracker/core/constants/app_loader_widget.dart';
import 'package:expense_tracker/core/constants/gaps.dart';
import 'package:expense_tracker/core/extensions/common_extension.dart';
import 'package:expense_tracker/core/localization/translate.dart';
import 'package:expense_tracker/core/requester/consumer/requester_consumer.dart';
import 'package:expense_tracker/core/requester/consumer/requester_state_consumer.dart';
...
import 'package:expense_tracker/core/widgets/base_form_option/collection_helper/collection_helper.dart';
import 'package:expense_tracker/core/widgets/base_form_option/form_option_selection_calculator.dart';
import 'package:expense_tracker/core/widgets/base_form_option/options_listview/display_format_enum.dart';
import 'package:expense_tracker/core/widgets/base_form_option/options_listview/options_list_view.dart';
...
import 'package:expense_tracker/core/widgets/base_form_option/options_requester/option_controller.dart';
...
import 'package:expense_tracker/core/widgets/base_form_option/sheet/option_sheet_button.dart';
import 'package:expense_tracker/core/widgets/shimmers/base_shimmer_widget.dart';

typedef OptionItemBuilder<T> = Widget Function(T item, bool Function(T item) isItemSelected, ValueChanged<T> onSelect);
...
class BaseFormOption<T, S> extends StatefulWidget {
  ...
  static BaseFormOption singleLocalOption<T>({
    ...
    return BaseFormOption<T, String>(
        ...
        selectedOptionBuilder: selectedOptionBuilder != null ? (items) => selectedOptionBuilder(items?.first) : null,
...
class BaseFormOptionState<T, S> extends State<BaseFormOption<T, S>> {
  ...
  void initState() {
    ...
    _selectedItemCalculator = FormOptionSelectionCalculator<T, S>(formOptionWidget: widget, optionController: _optionController);
  ...
  void didUpdateWidget(covariant BaseFormOption<T, S> oldWidget) {
    ...
    if (CollectionHelper.isNotIdenticalOptionsList<T>(
        currentSelectedItems: _optionController.selectedValue, incomingSelectedItems: widget.selectedItems, valueIdGetter: widget.valueIdGetter)) {
      _selectedItemCalculator.reCalculateSelectedItems(widget.selectedItems ?? []);
    }
  }
        ...
        builder: (formState) {
          return Column(
            ...
            children: [
                ...
                child: RequesterStateConsumer(
                  ...
                  builder: (context, state) {
                    return OptionSheetButton<List<T>>(
                      constraints: BoxConstraints(minHeight: const BoxConstraints(minHeight: 42, maxHeight: 42).minHeight),
                      ...
                      optionSheetMode: widget.isMultiple ? OptionSheetMode.saveButton : OptionSheetMode.clickAndSave,
                      ...
                      onTap: () {
                        if (widget.optionsRequester.state is int) {
                          ...
                          widget.optionsRequester.resetOptionsWithSelected(_optionController.selectedValue);
                        }
                      },
                      content: (context) {
                        return RequesterConsumer<List<T>>(
                          ...
                          successBuilder: (context, items, isLoading) {
                            if (items.isNullOrEmpty && !isLoading) {
                              return Text(Translate.s.noResults, style: AppTextStyle.s13_w500(color: AppColors.noContextInstance.blackOpacity), textAlign: TextAlign.center);
                            }
              ...
              if (formState.hasError) ...[Gaps.vGap(4), Text(formState.errorText ?? '', style: AppTextStyle.s12_w400(color: AppColors.snackBarRedError))],
            ],
          );
        });
  }
  ...
  bool _isItemSelected(T item) {
    if (widget.isMultiple) {
      return ((_optionController.tempValue) ?? []).map((e) => widget.valueIdGetter(e)).toList().contains(widget.valueIdGetter(item));
    }
    return widget.valueIdGetter(item) == widget.valueIdGetter(_optionController.tempValue?.firstOrNull);
  }
  ...
  void _onSelect(T item) {
    if (widget.isMultiple) {
      ...
      if (_isItemSelected(item)) {
        ...
        tempList.removeWhere((element) => widget.valueIdGetter(element) == widget.valueIdGetter(item));
