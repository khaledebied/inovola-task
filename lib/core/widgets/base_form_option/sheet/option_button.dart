...
import 'package:expense_tracker/core/widgets/base_form_option/sheet/hint_text.dart';
import 'package:expense_tracker/core/widgets/base_form_option/sheet/option_button_decoration.dart';
...
class _OptionsButtonState<T> extends State<OptionsButton<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.showDecoration) {
                  ...
                  Expanded(
                    child: Row(
                      children: [
                        ...
                        /// Value
                        Expanded(child: _ValueBuilder(controller: widget.controller, hintText: widget.hintText, valueBuilder: widget.valueBuilder)),
                        customSuffixIcon(),
                      ],
                    ),
                  ),
      ...
      return GestureDetector(
        onTap: widget.onPressed,
        child: _ValueBuilder(controller: widget.controller, hintText: widget.hintText, valueBuilder: widget.valueBuilder),
      );
    }
  }
  ...
  Widget customSuffixIcon() {
    return BlocBuilder<OptionController<T>, OptionControllerState<T>>(
      ...
      builder: (context, _) {
        bool isNotAnEmptyList = (widget.controller.selectedValue is List ? (widget.controller.selectedValue as List).isNotEmpty : true);
        return Row(
          children: [
            if ((widget.controller.selectedValue != null && isNotAnEmptyList && widget.showClearIcon) || widget.showClearIconIfNoValue) ...[
              /// Clear icon
            ...
            /// Suffix
            widget.customSuffixIcon ?? Icon(Icons.keyboard_arrow_down_rounded, size: 20, color: AppColors.noContextInstance.blackOpacity),
          ],
        );
      },
    );
...
class _ValueBuilder<T> extends StatelessWidget {
  ...
  const _ValueBuilder({Key? key, required this.controller, required this.hintText, required this.valueBuilder}) : super(key: key);
