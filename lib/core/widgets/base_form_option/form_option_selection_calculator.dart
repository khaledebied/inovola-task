...
import 'package:expense_tracker/core/extensions/string_helper_extension.dart';
...
class FormOptionSelectionCalculator<T, S> {
  ...
  void _reassignSelectedItems(List<T>? calculatedSelectedItems) {
    if (CollectionHelper.isNotIdenticalOptionsList(
        valueIdGetter: formOptionWidget.valueIdGetter, incomingSelectedItems: calculatedSelectedItems, currentSelectedItems: optionController.selectedValue)) {
  ...
  bool get isThereAnySelectedNotResolved {
    return optionController.selectedValue?.any((element) => formOptionWidget.valueMainTitleGetter(element).isBlank) ?? true;
  }
  ...
  List<T>? _reCalculateSelectedItemsFromOption(List<T> options) {
    ...
    List<String?> selectedOptionIds = (optionController.selectedValue ?? []).map((e) => formOptionWidget.valueIdGetter(e)).toList();
