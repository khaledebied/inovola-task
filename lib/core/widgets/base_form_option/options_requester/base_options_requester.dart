...
import 'package:expense_tracker/core/requester/requester.dart';
import 'package:expense_tracker/core/widgets/base_form_option/options_requester/option_matchers/option_matcher.dart';
...
class BaseOptionsRequester<T, S> extends Requester<List<T>> {
  ...
  BaseOptionsRequester({required this.isRemotelySearch, required this.immediatelyRequestOptions, required this.optionMatcher, required this.fetcher}) {
  ...
  void resetOptionsWithSelected(List<T>? selected) {
    ...
    if (optionsRepository != null) {
      selectedNotIncluded = selected!.where((element) => !optionsRepository!.contains(element)).toList();
    }
