...
abstract class CollectionHelper {
  ...
  static bool isNotIdenticalOptionsList<T>({List<T>? incomingSelectedItems, List<T>? currentSelectedItems, required String? Function(T?) valueIdGetter}) {
    if (incomingSelectedItems.isNullOrEmpty && currentSelectedItems.isNetherNullNorEmpty) {
      return true;
    }
    List<String> widgetSelectedItemsIds = incomingSelectedItems?.map((e) => valueIdGetter(e)).whereType<String>().toList() ?? [];
    List<String> optionControllerSelectedItemsIds = currentSelectedItems?.map((e) => valueIdGetter(e)).whereType<String>().toList() ?? [];
    if (widgetSelectedItemsIds.length == optionControllerSelectedItemsIds.length && widgetSelectedItemsIds.every((element) => optionControllerSelectedItemsIds.contains(element))) {
      return false;
    }
    return true;
  }
  ...
  /// }
  /// ```
  static Map<String, dynamic>? flattenNestedJson(Map<dynamic, dynamic>? map, [bool ignoreLocalizedCase = false]) {
